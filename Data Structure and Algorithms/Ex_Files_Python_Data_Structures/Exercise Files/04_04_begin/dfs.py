...
def dfs(maze, start, goal):
    stack=Stack()
    stack.push(start)
    predecessors ={start:None}#dictionary
    
    while not stack.is_empty():
        current_cell=stack.pop()
        if current_cell==goal:
            return get_path(predecessors,start,goal)
        for direction in ['up','right','down','left']:#clockwise direction
          ...
          #  print(current_cell)
           # print(predecessors)
        print(get_path(predecessors, start, current_cell))
            #print_maze(maze,predecessors,current_cell,start,goal)
...
if __name__ == "__main__":
   ...
   # assert result == [(0, 0), (1, 0), (2, 0), (2, 1), (2, 2)]#if not equal to then error
